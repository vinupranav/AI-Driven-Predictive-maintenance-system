...
def main():
    ...
    lub_oil_pressure = st.slider("Lub Oil Pressure", min_value=custom_ranges['Lub oil pressure'][0],
                                 ...
                                 value=(custom_ranges['Lub oil pressure'][0] + custom_ranges['Lub oil pressure'][
                                     1]) / 2)
    ...
    coolant_pressure = st.slider("Coolant Pressure", min_value=custom_ranges['Coolant pressure'][0],
                                 ...
                                 value=(custom_ranges['Coolant pressure'][0] + custom_ranges['Coolant pressure'][
                                     1]) / 2)
    ...
    temp_difference = st.slider("Temperature Difference", min_value=custom_ranges['Temperature_difference'][0],
                                ...
                                value=(custom_ranges['Temperature_difference'][0] +
                                       custom_ranges['Temperature_difference'][1]) / 2)

    # Predict button
    if st.button("Predict Engine Condition"):
        result, confidence = predict_condition(engine_rpm, lub_oil_pressure, fuel_pressure, coolant_pressure,
                                               lub_oil_temp, coolant_temp, temp_difference)

        # Explanation
        if result == 0:
            st.info(
                f"The engine is predicted to be in a normal condition. As the  Confidence level of the machine is : {1.0 - confidence:.2%}")
        else:
            st.warning(
                f"Warning! Please investigate further,As the  Confidence level of the machine is : {1.0 - confidence:.2%}")
